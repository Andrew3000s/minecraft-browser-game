...
class WeatherSystem {    constructor(canvas, world, timeSystem, soundSystem = null) {
    ...
    updatePrecipitation(deltaTime) {
        ...
        if (shouldPrecipitate && this.precipitation.length < this.maxPrecipitation) {
    ...
    adjustCloudsForWeather(weather) {
        ...
        console.log(`🌤️ Weather render called: ${this.currentWeather}, intensity: ${this.weatherIntensity.toFixed(2)}, clouds: ${this.clouds.length}, precipitation: ${this.precipitation.length}`);
        console.log(`📷 Camera: x=${camera?.x?.toFixed(1) || 'undefined'}, y=${camera?.y?.toFixed(1) || 'undefined'}, zoom=${camera?.zoom?.toFixed(2) || 'undefined'}`);
        ...
        console.log(`🌥️ Actually rendered ${renderedClouds} clouds out of ${this.clouds.length}`);
        this.ctx.restore();
    }
      renderCloud(x, y, cloud) {
        // Debug first cloud rendering
        if (Math.random() < 0.01) {
            console.log(`🌥️ Rendering cloud at (${x.toFixed(1)}, ${y.toFixed(1)}) size: ${cloud.width}x${cloud.height} opacity: ${cloud.opacity}`);
        }
